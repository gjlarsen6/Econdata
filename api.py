...
OPTIONAL_RESULTS: dict[str, Path] = {
    ...
    # Weather models — national
    "weather_temperature_national":    OUTPUT_DIR / "results_weather_temperature_national.json",
    "weather_precipitation_national":  OUTPUT_DIR / "results_weather_precipitation_national.json",
    "weather_extremes_national":       OUTPUT_DIR / "results_weather_extremes_national.json",
    # Weather models — regions
    "weather_temperature_northeast":   OUTPUT_DIR / "results_weather_temperature_northeast.json",
    "weather_precipitation_northeast": OUTPUT_DIR / "results_weather_precipitation_northeast.json",
    "weather_extremes_northeast":      OUTPUT_DIR / "results_weather_extremes_northeast.json",
    "weather_temperature_midwest":     OUTPUT_DIR / "results_weather_temperature_midwest.json",
    "weather_precipitation_midwest":   OUTPUT_DIR / "results_weather_precipitation_midwest.json",
    "weather_extremes_midwest":        OUTPUT_DIR / "results_weather_extremes_midwest.json",
    "weather_temperature_south":       OUTPUT_DIR / "results_weather_temperature_south.json",
    "weather_precipitation_south":     OUTPUT_DIR / "results_weather_precipitation_south.json",
    "weather_extremes_south":          OUTPUT_DIR / "results_weather_extremes_south.json",
    "weather_temperature_west":        OUTPUT_DIR / "results_weather_temperature_west.json",
    "weather_precipitation_west":      OUTPUT_DIR / "results_weather_precipitation_west.json",
    "weather_extremes_west":           OUTPUT_DIR / "results_weather_extremes_west.json",
}
...
# ── Weather Pydantic models ───────────────────────────────────────────────────

class CloudForecastPoint(BaseModel):
    month: str
    mid: float
    lo: float
    hi: float
...
class CloudCoverResponse(BaseModel):
    ...
    last_date: str | None = None
    last_value: float | None = None
    last_sky_condition: str | None = None
    validation: ValidationMetrics | None = None
    forecast: list[CloudForecastPoint]


class WeatherCityEntry(BaseModel):
    state: str
    city: str


class WeatherCitiesResponse(BaseModel):
    city_count: int
    cities: list[WeatherCityEntry]


class WeatherAvailabilityEntry(BaseModel):
    geography: str
    geography_type: str
    temperature: bool
    precipitation: bool
...
class WeatherSummaryResponse(BaseModel):
    generated_at: str
    national: WeatherAvailabilityEntry
    regions: list[WeatherAvailabilityEntry]
    city_count: int
...
_ENDPOINT_DESCRIPTORS: list[dict] = [
    ...
    # ── Weather — National ────────────────────────────────────────────────────
    {
        "path": "/api/weather/temperature",
        "description": (
            "12-month LightGBM forecasts for US national temperature & energy demand: "
            "mean temperature, temp volatility, HDD, CDD, temp anomaly vs 2000-2019 baseline, "
            "and mean daily temp range. Generated by: python fred_refresh.py --weather"
        ),
        "group": "Weather: Temperature & Energy Demand",
        "key": "weather_temperature_national",
        "optional": True,
    },
    {
        "path": "/api/weather/precipitation",
        "description": (
            "12-month LightGBM forecasts for US national precipitation & disruption: "
            "total precip, peak daily precip, precip days, snowfall, extreme precip days, "
            "and max consecutive dry days. Generated by: python fred_refresh.py --weather"
        ),
        "group": "Weather: Precipitation & Disruption",
        "key": "weather_precipitation_national",
        "optional": True,
    },
    {
        "path": "/api/weather/extremes",
        "description": (
            "12-month LightGBM forecasts for US national weather extremes & wind: "
            "extreme heat/cold days, wind speed, and cloud cover. "
            "Generated by: python fred_refresh.py --weather"
        ),
        "group": "Weather: Extremes & Wind",
        "key": "weather_extremes_national",
        "optional": True,
    },
    {
        "path": "/api/weather/cloud-cover",
        "description": (
            "US national cloud cover forecast with sky condition labels "
            "(Clear / Mostly Sunny / Partly Cloudy / Mostly Cloudy / Overcast). "
            "Derived from the extremes model cloud_cover_mean series (0–100%). "
            "Generated by: python fred_refresh.py --weather"
        ),
        "group": "Weather: Cloud Cover",
        "key": "weather_extremes_national",
        "optional": True,
    },
    # ── Weather — Regional ────────────────────────────────────────────────────
    {
        "path": "/api/weather/region/{region}/temperature",
        "description": (
            "12-month temperature & energy demand forecasts by Census region "
            "(northeast / midwest / south / west). "
            "Generated by: python fred_refresh.py --weather"
        ),
        "group": "Weather: Temperature & Energy Demand",
        "key": "weather_temperature_northeast",
        "optional": True,
    },
    {
        "path": "/api/weather/region/{region}/precipitation",
        "description": (
            "12-month precipitation & disruption forecasts by Census region "
            "(northeast / midwest / south / west). "
            "Generated by: python fred_refresh.py --weather"
        ),
        "group": "Weather: Precipitation & Disruption",
        "key": "weather_precipitation_northeast",
        "optional": True,
    },
    {
        "path": "/api/weather/region/{region}/extremes",
        "description": (
            "12-month extremes & wind forecasts by Census region "
            "(northeast / midwest / south / west). "
            "Generated by: python fred_refresh.py --weather"
        ),
        "group": "Weather: Extremes & Wind",
        "key": "weather_extremes_northeast",
        "optional": True,
    },
    {
        "path": "/api/weather/region/{region}/cloud-cover",
        "description": (
            "Regional cloud cover forecast with sky condition labels by Census region "
            "(northeast / midwest / south / west). "
            "Generated by: python fred_refresh.py --weather"
        ),
        "group": "Weather: Cloud Cover",
    ...
    # ── Weather — City ────────────────────────────────────────────────────────
    {
        "path": "/api/weather/city/{state}/{city}/temperature",
        "description": (
            "12-month temperature & energy demand forecasts for a specific US city. "
            "Use /api/weather/cities to list available cities. "
            "Generated by: python fred_refresh.py --weather"
        ),
        "group": "Weather: Temperature & Energy Demand",
        "key": None,
    ...
    {
        "path": "/api/weather/city/{state}/{city}/precipitation",
        "description": (
            "12-month precipitation & disruption forecasts for a specific US city. "
            "Use /api/weather/cities to list available cities. "
            "Generated by: python fred_refresh.py --weather"
        ),
        "group": "Weather: Precipitation & Disruption",
    ...
    {
        "path": "/api/weather/city/{state}/{city}/extremes",
        "description": (
            "12-month extremes & wind forecasts for a specific US city. "
            "Use /api/weather/cities to list available cities. "
            "Generated by: python fred_refresh.py --weather"
        ),
        "group": "Weather: Extremes & Wind",
    ...
    {
        "path": "/api/weather/city/{state}/{city}/cloud-cover",
        "description": (
            "City-level cloud cover forecast with sky condition labels. "
            "Use /api/weather/cities to list available cities. "
            "Generated by: python fred_refresh.py --weather"
        ),
        "group": "Weather: Cloud Cover",
    ...
    # ── Weather — Discovery ───────────────────────────────────────────────────
    {
        ...
        "description": "List all US cities that have weather model results available.",
        "group": None,
        "key": None,
        "optional": True,
    },
    {
        "path": "/api/weather/cities/{state}",
        "description": "List cities in a specific US state that have weather model results available.",
        "group": None,
        "key": None,
        "optional": True,
    },
    {
        ...
        "description": (
            "Availability matrix for all weather model geographies "
            "(national, 4 regions, city count) across the 3 forecast groups."
        ),
        "group": None,
        "key": None,
        "optional": True,
...
def get_summary() -> SummaryResponse:
    ...
    for desc in _ENDPOINT_DESCRIPTORS:
        ...
        if key is None:
            if desc["path"].startswith("/api/weather/"):
                # Weather city/discovery endpoints — available if city outputs dir has content
                city_dir = OUTPUT_DIR / "city"
                available = city_dir.is_dir() and any(city_dir.iterdir())
            else:
            ...
            if available:
                try:
                    ...
                    log.warning("summary: could not parse %s — %s", path_obj.name, exc)
...
_VALID_REGIONS = {"northeast", "midwest", "south", "west"}
_WEATHER_SERIES_RE = re.compile(r"^[a-zA-Z0-9_]{1,40}$")


def _sky_condition(pct: float) -> str:
    """Map a cloud cover percentage (0–100) to a sky condition label."""
    if pct < 20:
        return "Clear"
    if pct < 40:
        return "Mostly Sunny"
...
def _weather_city_path(state: str, city: str, group: str) -> Path:
    return OUTPUT_DIR / "city" / state / city / f"results_weather_{group}_{city}.json"


def _validate_weather_city(state: str, city: str) -> tuple[str, str]:
    """Uppercase state, strip city, reject invalid inputs. Raises 422 on failure."""
    state = state.upper()
    if not re.match(r"^[A-Z]{2}$", state):
        raise HTTPException(
            status_code=422,
            detail="state must be a 2-letter US state code (e.g. CA, TX).",
        )
    city = city.strip()
    if not city or len(city) > 60 or ".." in city or "\\" in city:
        raise HTTPException(
            status_code=422,
            detail="city must be 1–60 characters with no path traversal sequences.",
        )
    return state, city


def _find_weather_series(group: GroupResponse, series_id: str) -> SeriesResult:
    """Like _find_series but handles lowercase weather series_ids (e.g. temp_mean, hdd)."""
    if not _WEATHER_SERIES_RE.match(series_id):
        raise HTTPException(
            status_code=400,
            detail="series_id must be 1–40 alphanumeric characters or underscores.",
        )
    sid_lower = series_id.lower()
    match = next((s for s in group.series if s.series_id.lower() == sid_lower), None)
    if match is None:
        available = [s.series_id for s in group.series]
        raise HTTPException(
            status_code=404,
            detail=f"Series '{series_id}' not found in group '{group.group}'. Available: {available}",
        )
    return match


def _build_cloud_cover_response(group: GroupResponse, geography: str) -> CloudCoverResponse:
    """Extract cloud_cover_mean from an extremes GroupResponse and annotate with sky conditions."""
    cc = next((s for s in group.series if s.series_id == "cloud_cover_mean"), None)
    if cc is None:
        raise HTTPException(
            status_code=404,
            detail=f"cloud_cover_mean not found in extremes results for '{geography}'.",
        )
    return CloudCoverResponse(
        ...
        last_value=cc.last_value,
        last_sky_condition=_sky_condition(cc.last_value) if cc.last_value is not None else None,
        ...
        forecast=[
            CloudForecastPoint(
                ...
                lo=pt.lo,
                hi=pt.hi,
                sky_condition=_sky_condition(pt.mid),
            )
            for pt in cc.forecast
        ],
    )


def _list_weather_cities(state_filter: str | None = None) -> list[WeatherCityEntry]:
    """Scan outputs/city/ and return (state, city) pairs that have at least one weather results file."""
    city_dir = OUTPUT_DIR / "city"
    if not city_dir.exists():
        return []
    entries: list[WeatherCityEntry] = []
    state_dirs = sorted(city_dir.iterdir()) if state_filter is None else [city_dir / state_filter.upper()]
    for state_path in state_dirs:
        ...
        for city_path in sorted(state_path.iterdir()):
            if not city_path.is_dir():
                continue
            if any(city_path.glob("results_weather_*.json")):
                entries.append(WeatherCityEntry(state=state_path.name, city=city_path.name))
    return entries
...
@app.get(
    ...
    response_model=GroupResponse,
    tags=["Weather Models"],
    summary="US national temperature & energy demand forecasts",
)
def get_weather_temperature_national() -> GroupResponse:
    """
    12-month LightGBM forecasts for US national temperature & energy demand:
    - **temp_mean** — Mean Temperature (°F)
    - **temp_std** — Temperature Volatility (°F std dev)
    - **hdd** — Heating Degree Days (base 65°F)
    - **cdd** — Cooling Degree Days (base 65°F)
    - **temp_anom** — Temperature Anomaly vs 2000–2019 baseline (°F)
    - **temp_range_mean** — Mean Daily Temperature Range (°F)

    **Requires prior run:** `python fred_refresh.py --weather`
    """
    return _load_result(
        OPTIONAL_RESULTS["weather_temperature_national"],
        "weather_temperature_national",
        optional=True,
    )


@app.get(
    "/api/weather/temperature/{series_id}",
    response_model=SeriesResult,
    tags=["Weather Models"],
    summary="Single national temperature series forecast",
)
def get_weather_temperature_national_series(series_id: str) -> SeriesResult:
    """
    Single series from the national temperature group.

    Valid `series_id` values: **temp_mean**, **temp_std**, **hdd**, **cdd**,
    **temp_anom**, **temp_range_mean**
    ...
    **Requires prior run:** `python fred_refresh.py --weather`
    ...
    return _find_weather_series(group, series_id)
...
@app.get(
    ...
    response_model=GroupResponse,
    tags=["Weather Models"],
    summary="US national precipitation & disruption forecasts",
)
def get_weather_precipitation_national() -> GroupResponse:
    """
    12-month LightGBM forecasts for US national precipitation & disruption:
    - **precip_total** — Monthly Precipitation (in)
    - **precip_max_day** — Peak Daily Precipitation (in)
    - **precip_days** — Precipitation Days (count)
    - **snow_total** — Snow Depth (in)
    - **extreme_precip_days** — Extreme Precip Days >1 in (count)
    - **max_consec_dry** — Max Consecutive Dry Days (count)

    **Requires prior run:** `python fred_refresh.py --weather`
    """
    return _load_result(
        OPTIONAL_RESULTS["weather_precipitation_national"],
        "weather_precipitation_national",
        optional=True,
    )


@app.get(
    "/api/weather/precipitation/{series_id}",
    response_model=SeriesResult,
    tags=["Weather Models"],
    summary="Single national precipitation series forecast",
)
def get_weather_precipitation_national_series(series_id: str) -> SeriesResult:
    """
    Single series from the national precipitation group.

    Valid `series_id` values: **precip_total**, **precip_max_day**, **precip_days**,
...
@app.get(
    ...
    response_model=GroupResponse,
    tags=["Weather Models"],
    summary="US national weather extremes & wind forecasts",
)
def get_weather_extremes_national() -> GroupResponse:
    """
    12-month LightGBM forecasts for US national weather extremes & wind:
    - **extreme_heat_days** — Days >90°F (count)
    - **extreme_heat_95_days** — Days >95°F (count)
    - **extreme_cold_days** — Days <32°F (count)
    - **extreme_cold_20_days** — Days <20°F (count)
    ...
    **Requires prior run:** `python fred_refresh.py --weather`
    """
    return _load_result(
        OPTIONAL_RESULTS["weather_extremes_national"],
        "weather_extremes_national",
        optional=True,
    )


@app.get(
    "/api/weather/extremes/{series_id}",
    response_model=SeriesResult,
    tags=["Weather Models"],
    summary="Single national extremes series forecast",
)
def get_weather_extremes_national_series(series_id: str) -> SeriesResult:
    """
    Single series from the national extremes group.

    Valid `series_id` values: **extreme_heat_days**, **extreme_heat_95_days**,
    **extreme_cold_days**, **extreme_cold_20_days**, **wind_mean**, **wind_max**,
    **cloud_cover_mean**
...
@app.get(
    ...
    tags=["Weather Models"],
    summary="US national cloud cover forecast with sky condition labels",
)
def get_weather_cloud_cover_national() -> CloudCoverResponse:
    """
    12-month forecast for US national **cloud_cover_mean** (%) with a
    `sky_condition` label on each forecast point:
    ...
    **Requires prior run:** `python fred_refresh.py --weather`
    ...
    return _build_cloud_cover_response(group, "national")


# ── Weather — Regional ────────────────────────────────────────────────────────

@app.get(
    "/api/weather/region/{region}/temperature",
    response_model=GroupResponse,
    tags=["Weather Models"],
    summary="Regional temperature & energy demand forecasts",
)
def get_weather_region_temperature(region: str) -> GroupResponse:
    """
    12-month temperature & energy demand forecasts for a US Census region.
    ...
    **Requires prior run:** `python fred_refresh.py --weather`
    ...
    if region not in _VALID_REGIONS:
        raise HTTPException(
            status_code=422,
            detail=f"region must be one of: {sorted(_VALID_REGIONS)}",
        )
    key = f"weather_temperature_{region}"
    return _load_result(OPTIONAL_RESULTS[key], key, optional=True)


@app.get(
    "/api/weather/region/{region}/precipitation",
    response_model=GroupResponse,
    tags=["Weather Models"],
    summary="Regional precipitation & disruption forecasts",
)
def get_weather_region_precipitation(region: str) -> GroupResponse:
    """
    12-month precipitation & disruption forecasts for a US Census region.
    ...
    **Requires prior run:** `python fred_refresh.py --weather`
    ...
    key = f"weather_precipitation_{region}"
    return _load_result(OPTIONAL_RESULTS[key], key, optional=True)


@app.get(
    "/api/weather/region/{region}/extremes",
    response_model=GroupResponse,
    tags=["Weather Models"],
    summary="Regional weather extremes & wind forecasts",
)
def get_weather_region_extremes(region: str) -> GroupResponse:
    """
    12-month extremes & wind forecasts for a US Census region.
    ...
    **Requires prior run:** `python fred_refresh.py --weather`
    ...
    key = f"weather_extremes_{region}"
    return _load_result(OPTIONAL_RESULTS[key], key, optional=True)


@app.get(
    "/api/weather/region/{region}/cloud-cover",
    response_model=CloudCoverResponse,
    tags=["Weather Models"],
    summary="Regional cloud cover forecast with sky condition labels",
)
def get_weather_region_cloud_cover(region: str) -> CloudCoverResponse:
    """
    12-month cloud cover forecast with sky condition labels for a US Census region.
    ...
    **Requires prior run:** `python fred_refresh.py --weather`
    ...
    key = f"weather_extremes_{region}"
    group = _load_result(OPTIONAL_RESULTS[key], key, optional=True)
    return _build_cloud_cover_response(group, region)


# ── Weather — City ────────────────────────────────────────────────────────────

@app.get(
    "/api/weather/city/{state}/{city}/temperature",
    response_model=GroupResponse,
    tags=["Weather Models"],
    summary="City-level temperature & energy demand forecasts",
)
def get_weather_city_temperature(state: str, city: str) -> GroupResponse:
    """
    12-month temperature & energy demand forecasts for a specific US city.

    - `state` — 2-letter state code (e.g. **CA**, **TX**)
    - `city` — city name, URL-encoded if it contains spaces (e.g. `Los%20Angeles`)

    Use `/api/weather/cities` to list all available cities.

    **Requires prior run:** `python fred_refresh.py --weather`
    """
    state, city = _validate_weather_city(state, city)
    path = _weather_city_path(state, city, "temperature")
    if not path.exists():
        raise HTTPException(
            status_code=404,
            detail=f"No temperature weather results for {state}/{city}. Use /api/weather/cities to list available cities.",
        )
    return _load_result(path, f"weather_temperature_{state}_{city}", optional=True)


@app.get(
    "/api/weather/city/{state}/{city}/precipitation",
    response_model=GroupResponse,
    tags=["Weather Models"],
    summary="City-level precipitation & disruption forecasts",
)
def get_weather_city_precipitation(state: str, city: str) -> GroupResponse:
    """
    12-month precipitation & disruption forecasts for a specific US city.
    ...
    **Requires prior run:** `python fred_refresh.py --weather`
    """
    state, city = _validate_weather_city(state, city)
    path = _weather_city_path(state, city, "precipitation")
    if not path.exists():
        raise HTTPException(
            status_code=404,
            detail=f"No precipitation weather results for {state}/{city}. Use /api/weather/cities to list available cities.",
        )
    return _load_result(path, f"weather_precipitation_{state}_{city}", optional=True)


@app.get(
    "/api/weather/city/{state}/{city}/extremes",
    response_model=GroupResponse,
    tags=["Weather Models"],
    summary="City-level weather extremes & wind forecasts",
)
def get_weather_city_extremes(state: str, city: str) -> GroupResponse:
    """
    12-month extremes & wind forecasts for a specific US city.
    ...
    **Requires prior run:** `python fred_refresh.py --weather`
    """
    state, city = _validate_weather_city(state, city)
    path = _weather_city_path(state, city, "extremes")
    if not path.exists():
        raise HTTPException(
            status_code=404,
            detail=f"No extremes weather results for {state}/{city}. Use /api/weather/cities to list available cities.",
        )
    return _load_result(path, f"weather_extremes_{state}_{city}", optional=True)


@app.get(
    "/api/weather/city/{state}/{city}/cloud-cover",
    response_model=CloudCoverResponse,
    tags=["Weather Models"],
    summary="City-level cloud cover forecast with sky condition labels",
)
def get_weather_city_cloud_cover(state: str, city: str) -> CloudCoverResponse:
    """
    12-month cloud cover forecast with sky condition labels for a specific US city.
    ...
    **Requires prior run:** `python fred_refresh.py --weather`
    ...
    group = _load_result(path, f"weather_extremes_{state}_{city}", optional=True)
    return _build_cloud_cover_response(group, f"{state}/{city}")
...
@app.get(
    "/api/weather/cities",
    response_model=WeatherCitiesResponse,
    tags=["Weather Models"],
    summary="List all US cities with weather model results",
)
def get_weather_cities() -> WeatherCitiesResponse:
    """
    Returns all US cities that have at least one weather model results file in
    `outputs/city/`. Sorted by state then city name.

    Each entry has `state` (2-letter code) and `city` (name). Use these values
    to construct city-level weather endpoint paths.
    """
    entries = _list_weather_cities()
    return WeatherCitiesResponse(city_count=len(entries), cities=entries)


@app.get(
    "/api/weather/cities/{state}",
    response_model=WeatherCitiesResponse,
    tags=["Weather Models"],
    summary="List cities in a US state with weather model results",
)
def get_weather_cities_by_state(state: str) -> WeatherCitiesResponse:
    """
    Returns cities in a given US state that have weather model results.

    - `state` — 2-letter state code (e.g. **CA**, **TX**)

    Returns 404 if no results exist for the state.
    """
    state_upper = state.upper()
    if not re.match(r"^[A-Z]{2}$", state_upper):
        raise HTTPException(
            status_code=422,
            detail="state must be a 2-letter US state code (e.g. CA, TX).",
        )
    entries = _list_weather_cities(state_filter=state_upper)
    if not entries:
        raise HTTPException(
            status_code=404,
            detail=f"No weather model results found for state '{state_upper}'. Run: python fred_refresh.py --weather",
        )
    return WeatherCitiesResponse(city_count=len(entries), cities=entries)
...
@app.get(
    ...
    tags=["Weather Models"],
    summary="Weather model availability across all geographies",
)
def get_weather_summary() -> WeatherSummaryResponse:
    """
    Availability matrix for all weather model geographies:
    - **national** — whether national results exist for each of the 3 groups
    - **regions** — whether regional results exist for northeast / midwest / south / west
    - **city_count** — number of cities with at least one results file
    """
    def _avail(key: str) -> bool:
        return OPTIONAL_RESULTS[key].exists()

    national = WeatherAvailabilityEntry(
        geography="national",
        geography_type="national",
        temperature=_avail("weather_temperature_national"),
        precipitation=_avail("weather_precipitation_national"),
        extremes=_avail("weather_extremes_national"),
    )
    regions = [
        WeatherAvailabilityEntry(
            geography=r,
            geography_type="region",
            temperature=_avail(f"weather_temperature_{r}"),
            precipitation=_avail(f"weather_precipitation_{r}"),
            extremes=_avail(f"weather_extremes_{r}"),
        )
        for r in sorted(_VALID_REGIONS)
    ]
    city_count = len(_list_weather_cities())
